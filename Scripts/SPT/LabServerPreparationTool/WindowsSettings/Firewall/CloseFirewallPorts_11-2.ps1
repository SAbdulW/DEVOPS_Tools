...
$siteIPs = Import-Csv ".\SitesIPReleations.csv" -Header "ForPlatform","MySiteIps","AllSitesIPs" -Delimiter ":"
$a = Import-Csv  ".\FirewallPorts_11-2.csv" -Header "Platform","Name","Protocol","Port","Scope"
$a | 
where { $_.Platform -eq $Platform} | 
foreach {
	$localport=$_.Port; 
	...
	$mySiteIps = $siteIPs | where { $_.ForPlatform -eq $Platform } | select MySiteIps
	$allSitesIps = $siteIPs | where { $_.ForPlatform -eq $Platform } | select AllSitesIPs
	echo $scope; 
	$myAllCorrectSiteIPs = $allSitesIps.AllSitesIPs
	$mySiteCorrectIPs = $mySiteIps.MySiteIps
	if($osVersion.Contains("2008") -or $osVersion.Contains("Windows 7") -or $osVersion.Contains("Windows 8") -or $osVersion.Contains("2012"))
	{
		if ($scope -eq "All") { 
		netsh advfirewall firewall delete rule name="WFO $rulename" dir=in profile=any localport="$localport" protocol="$protocol" 
		} 
		elseif ($scope -eq "Multi-Site") {
		netsh advfirewall firewall add rule name="WFO $rulename" dir=in action=allow profile=any remoteip="$myAllCorrectSiteIPs" localport="$localport" protocol="$protocol" 
		}
	
		else { 
		netsh advfirewall firewall add rule name="WFO"* dir=in action=allow profile=any remoteip="$mySiteCorrectIPs" localport="$localport" protocol="$protocol" 
		} 
	}
    else{
		if ($scope -eq "All") { 
		netsh firewall add portopening protocol="$protocol" port="$localport" name="I360"* mode=ENABLE scope=ALL 
		} 
		elseif ($scope -eq "Multi-Site") {
		netsh firewall add portopening protocol="$protocol" port="$localport" name="I360"* mode=ENABLE scope=CUSTOM addresses="$myAllCorrectSiteIPs" 
		}
	
		else { 
			netsh firewall add portopening protocol="$protocol" port="$localport" name="I360"* mode=ENABLE scope=CUSTOM addresses="$mySiteCorrectIPs"
		} 
	}

 }
 
 netsh advfirewall firewall delete rule dir=in name="WFO Block Port 80" localport=80 remoteport=80 protocol=tcp
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBUWyPgFV0Dw/GN
# fbX5m2Le7n9fKCSrKdv4Lc9S7MRCgqCCGS8wggPuMIIDV6ADAgECAhB+k+v7fMZO
...
# gjcCARUwLwYJKoZIhvcNAQkEMSIEIJdM80trMkbrJZsrzxKW1HgwImwgf3V2WqgT
# FSqqTCyIMA0GCSqGSIb3DQEBAQUABIIBACevJPSwj8J9yI/VN+3RMEhltrCEWvQu
# 0p/D6ZkKC55DcZy8FK1AZw1sua8Ijf9uaD3XXQDgvdctnjadWQf92S8DrHh1kwIM
# y7+Hr38hcyPhC5n86Onlve43KLvetKXTl7hWkE1egjLSDXsLZwVFC++vqHFftNCi
# HhUnmp+5AEG1jnapbq41SaT4OOU02pghx2YrxmoCE8fr5Bv2s+OL/rmwb3Obek9t
# /3QhOhF+BXfcx4kFDAU8Or1K2cxJU+nqcoH5voU80XTC3yVu3cZBchNNld3d08Zk
# v2CBV7vArvpOgZpGx3P4/XijpNfpoZjzWqqYD49bvfvK2AE2vR2WdtKhggILMIIC
...
# DxcNMTYwODIyMTU1NTU5WjAjBgkqhkiG9w0BCQQxFgQU1asOHuXqRjGAblROE0SG
# IEJJ0X4wDQYJKoZIhvcNAQEBBQAEggEAbcs5QmTQuQ/43JHvuttPqZgywp9iKhuC
# vOMyXa8wc31JEwebUqbJ5NzHkT6KRJIGdlahHek86gzQKETuMGknac1F6BxJMidI
# Tzrkne3hksQutAn6/KRCN44ECXTAaHQ9SCO7QT8tdEl3XRz1/9jQDNVBPVvpwjur
# sX1mKoSwLLuSQ5UzYfrJqPSZzhRsDBKxHfLzqDTW0NzeOGqp9+vwPZxYvShgPNr+
# KQvhc2SZ+YC739syPs95DNBKd4tOYS1+ZM9MXqqAY1ZchAZWpFlx25WS/r4sCpID
# K6wV9MzXOgUwmzWCvpuxyMaeL2gzFwYkfJa2lNFA63AhZeCJJ1refQ==
